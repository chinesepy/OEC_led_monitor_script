...
NIGHT_CHECK_INTERVAL=60    # 夜间检查间隔（秒）
...
while true; do
    ...
    if [ $? -eq 0 ]; then
        # 夜间模式
        if [ "$night_mode_logged" = "false" ]; then
            log_message "Entering night mode - LED off, reduced monitoring"
            night_mode_logged=true
        fi
        sleep $NIGHT_CHECK_INTERVAL
    else
        # 正常模式
        if [ "$night_mode_logged" = "true" ]; then
            log_message "Exiting night mode - resuming normal monitoring"
            night_mode_logged=false
        fi
